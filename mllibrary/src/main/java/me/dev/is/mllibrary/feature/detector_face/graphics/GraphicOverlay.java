...
public class GraphicOverlay extends View {
  ...
  public abstract static class Graphic {
    ...
    public float translateX(float x) {
      if (overlay.isImageFlipped) {
        final float translateX = (overlay.getWidth() - (scale(x) - overlay.postScaleWidthOffset));
        Log.d(TAG, "translateX():Flipped:Y, returned: " + translateX);
        return translateX;
      } else {
  ...
  private void updateTransformationIfNeeded() {
    ...
    float viewAspectRatio = (float) getWidth() / getHeight();//ex:viewAspectRatio =(640/480)=> v=1.33
    float imageAspectRatio = (float) imageWidth / imageHeight;//ex:imageAspectRatio=(480/480)=> i=1
