...
public class FaceGraphic extends GraphicOverlay.Graphic {
    ...
    private static final float BOX_STROKE_WIDTH = 3.0f;//5
    ...
    public void draw(Canvas canvas) {
        ...
        int colorID = getFaceColorId(face);

        // Calculate width and height of label box
        float textWidth = calculateTextWidth(colorID, "ID: " + face.getTrackingId());

        if (face.getSmilingProbability() != null) {
            yLabelOffset -= lineHeight;
            Log.d("TAG", "draw() called with: yLabelOffset = " + yLabelOffset + "");
            textWidth = Math.max(textWidth, calculateTextWidth(colorID, format("Happiness: %.2f", face.getSmilingProbability())));
        }
        if (face.getLeftEyeOpenProbability() != null) {
            yLabelOffset -= lineHeight;
            textWidth = Math.max(textWidth, calculateTextWidth(colorID, format("Left eye open: %.2f", face.getLeftEyeOpenProbability())));
        }
        if (face.getRightEyeOpenProbability() != null) {
            yLabelOffset -= lineHeight;
            textWidth = Math.max(textWidth, calculateTextWidth(colorID, format("Right eye open: %.2f", face.getRightEyeOpenProbability())));
        }

        yLabelOffset = yLabelOffset - 3 * lineHeight;
        textWidth = Math.max(textWidth, calculateTextWidth(colorID, format("EulerX: %.2f", face.getHeadEulerAngleX())));
        textWidth = Math.max(textWidth, calculateTextWidth(colorID, format("EulerY: %.2f", face.getHeadEulerAngleY())));
        textWidth = Math.max(textWidth, calculateTextWidth(colorID, format("EulerZ: %.2f", face.getHeadEulerAngleZ())));
        ...
        if (face.getSmilingProbability() != null) {
            canvas.drawText(format("Smiling: %.2f", face.getSmilingProbability()),
                    left,
                    top + yLabelOffset,
                    idPaints[colorID]);
            yLabelOffset += lineHeight;
        ...
        if (face.getLeftEyeOpenProbability() != null) {
            canvas.drawText(format("Left eye open: %.2f", face.getLeftEyeOpenProbability()),
                    left,
                    top + yLabelOffset,
                    idPaints[colorID]);
            yLabelOffset += lineHeight;
        ...
        //draw face orientation
        drawText(canvas, "EulerX: " + face.getHeadEulerAngleX(), left, top + yLabelOffset, idPaints[colorID]);
        yLabelOffset += lineHeight;
        drawText(canvas, "EulerY: " + face.getHeadEulerAngleY(), left, top + yLabelOffset, idPaints[colorID]);
        yLabelOffset += lineHeight;
        canvas.drawText("EulerZ: " + face.getHeadEulerAngleZ(), left, top + yLabelOffset, idPaints[colorID]);
        yLabelOffset += lineHeight;
        // Determine the face orientation based on Euler angles
        String faceOrientation = getFaceOrientation(face);
        // Draw the face orientation text
        drawText(canvas, "Orientation: " + faceOrientation, left, top + yLabelOffset, idPaints[colorID]);
    ...
    @NotNull
    private String getFaceOrientation(@NotNull Face face) {
        float eulerX = face.getHeadEulerAngleX();
        float eulerY = face.getHeadEulerAngleY();
        //float eulerZ = face.getHeadEulerAngleZ();

        if (eulerX > 12) return "Up";
        else if (eulerX < -12) return "Down";
        else if (eulerY < -12) return "Left";
        else if (eulerY > 12) return "Right";
        else if (Math.abs(eulerX) > 10 && Math.abs(eulerY) > 10) return "Tilt";
        else return "Straight";//return "Neutral";
    }


    private void drawFaceContours(Canvas canvas) {
        Log.w("TAG", "drawFaceContours() returned: " + face.getAllContours());
    ...
    private float calculateTextWidth(int colorID, String text) {
        return idPaints[colorID].measureText(text);
    }
    ...
    private int getFaceColorId(@NotNull Face face) {
        return (face.getTrackingId() == null) ? 0 : Math.abs(face.getTrackingId() % NUM_COLORS);
    }
    //================================================//
