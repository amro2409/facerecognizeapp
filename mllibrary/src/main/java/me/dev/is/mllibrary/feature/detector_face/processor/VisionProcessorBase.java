...
import me.dev.is.mllibrary.feature.detector_face.camera.CameraImageGraphic;
...
import me.dev.is.mllibrary.feature.detector_face.utils.FrameMetadata;
import me.dev.is.mllibrary.feature.detector_face.utils.PreferenceUtils;
import me.dev.is.mllibrary.feature.detector_face.utils.ScopedExecutor;
import me.dev.is.mllibrary.feature.detector_face.utils.TemperatureMonitor;
...
public abstract class VisionProcessorBase<T> implements VisionImageProcessor {
    ...
    // -----------------Code for processing single still(fixed) image----------------------------------------
    @Override
    ...
    public void processImageProxy(ImageProxy image, GraphicOverlay graphicOverlay) {
        ...
        if (isMlImageEnabled(graphicOverlay.getContext())) {
            MlImage mlImage = new MediaMlImageBuilder(Objects.requireNonNull(image.getImage()))
    ...
    private Task<T> requestDetectInImage(
            ...
            long frameStartMs) {
        return setUpListener(detectInImage(image,originalCameraImage), graphicOverlay, originalCameraImage, shouldShowFps, frameStartMs);
    }

    private Task<T> requestDetectInImage(final MlImage image, final GraphicOverlay graphicOverlay, @Nullable final Bitmap originalCameraImage,
    ...
    private Task<T> setUpListener(Task<T> task, final GraphicOverlay graphicOverlay, @Nullable final Bitmap originalCameraImage,
            boolean shouldShowFps,
            long frameStartMs) {
        ...
        return task.addOnSuccessListener(
                ...
                results -> {
                    ...
                    if (numRuns >= 500) {
                       VisionProcessorBase.this.resetLatencyStats();
                    }
                    numRuns++;
                    ...
                    if (frameProcessedInOneSecondInterval == 1) {
                        ...
                        Log.d(TAG, "Frame latency: max="
                                + maxFrameMs
                                + ", min="
                                + minFrameMs
                                + ", avg="
                                + totalFrameMs / numRuns);
                        Log.d(TAG, "Detector latency: max="
                                + maxDetectorMs
                                + ", min="
                                + minDetectorMs
                                + ", avg="
                        ...
                        long availableMegs = mi.availMem / 0x100000;//1048576
                   ...
                   VisionProcessorBase.this.onSuccess(results, graphicOverlay);
                    Log.d(TAG, "setUpListener() called with: task = [" + task + "], graphicOverlay = [" + graphicOverlay + "], originalCameraImage = [" + originalCameraImage + "], shouldShowFps = [" + shouldShowFps + "], frameStartMs = [" + frameStartMs + "]");
                    if (!PreferenceUtils.shouldHideDetectionInfo(graphicOverlay.getContext())) {
                        graphicOverlay.add(new InferenceInfoGraphic(
                                graphicOverlay,
    ...
    protected  Task<T> detectInImage(InputImage image, Bitmap btp){
        return Tasks.forException(new MlKitException("Image is currently not demonstrated for this feature", MlKitException.INVALID_ARGUMENT));
    }
