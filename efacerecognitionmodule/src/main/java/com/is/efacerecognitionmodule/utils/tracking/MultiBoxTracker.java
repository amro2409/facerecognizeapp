...
package com.is.efacerecognitionmodule.utils.tracking;
...
public class MultiBoxTracker {
  ...
  public synchronized void trackResults(@NotNull final List<Recognition> results, final long timestamp) {
  ...
  public synchronized void draw(@NotNull final Canvas canvas) {
    ...
    for (final TrackedRecognition recognition : trackedObjects) {
      ...
      float cornerSize = Math.min(trackedPos.width(), trackedPos.height()) / 4.0f;
      ...
      borderedText.drawText(
          canvas, trackedPos.left + cornerSize, trackedPos.top-40, labelString, boxPaint);
    }
  }
  ...
  private void processResults(final List<Recognition> results) {
    ...
    for (final Pair<Float, Recognition> potential : rectsToTrack) {
      ...
      if (potential.second.getColor() != 0) {
